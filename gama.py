...
from exchange1 import poloniex
from exchange2 import bittrex

from configparser import ConfigParser, NoSectionError

def main(argv):
	# Setup Argument Parser
	parser = argparse.ArgumentParser(description='Gamakuchi Arbitrage Bot')
	parser.add_argument('-s', '--symbol', default='XMR', type=str, required=False, help='symbol of your target coin [default: BTC]')
	...
	parser.add_argument('-c', '--config', default='gamakuchi.conf', type=str, required=False, help='config file [default: gamakuchi.conf]')
	parser.add_argument('-l', '--logfile', default='gamakuchi.log', type=str, required=False, help='file to output log data to [default: gamakuchi.log]')
	...
	config = ConfigParser()
	try:
        #### API KEY FROM API SECRET ####
		config.read(args.config)
		poloniexKey = config.get('Gamakuchi', 'poloniexKey')
		poloniexSecret = config.get('Gamakuchi', 'poloniexSecret')
		bittrexKey = config.get('Gamakuchi', 'bittrexKey')
		bittrexSecret = config.get('Gamakuchi', 'bittrexSecret')
		...
		config.add_section('GamakuchiBot')
		config.set('GamakuchiBot', 'poloniexKey', poloniexKey)
		config.set('GamakuchiBot', 'poloniexSecret', poloniexSecret)
		config.set('GamakuchiBot', 'bittrexKey', bittrexKey)
		config.set('GamakuchiBot', 'bittrexSecret', bittrexSecret)
		try:
	...
	def trade(_buyExchange, _ask, _bid, _sellBalance, _buyBalance):
		...
		if arbitrage <= args.rate:
			...
			buyExchangeString = 'Binance'
			sellExchangeString = 'Coincheck'
			...
			buyExchangeString = 'Coincheck'
			sellExchangeString = 'Binance'
